...
async function confirmCompletion() {
  ...
  // Coletar dados antes do loading
  ...
  const signature   = currentValidationType === 'signature'
    ...
    : null;
  const arquivos    = currentValidationType === 'photo'
  ...
  try {
    // 1. Atualizar dados textuais na planilha (situação, horário, assinante, COREN)
    document.getElementById('syncMessage').textContent = 'Salvando atividade...';
    await atualizarViaWebApp(activeActivity.rowIndex, true, signerName, signerCoren);

    // 2. Enviar evidências ao Drive (comprime + faz POST + aguarda processamento)
    document.getElementById('syncMessage').textContent = 'Enviando evidências...';
    await enviarEvidencias(activeActivity, currentValidationType, arquivos);

    // 3. Salvar localmente (localStorage como cache das evidências)
    ...
    // 4. Atualizar objeto local para refletir imediatamente na lista
