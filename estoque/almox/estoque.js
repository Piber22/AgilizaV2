...
// CONFIGURAÇÕES
// =============================
const sheetCSVUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vT5u8QMqykpJWy15fFvkhU_39Okn6VnwvffnBfi2lx_ezLxz5rdMXqTvK0D9RdN9kQQcPAGYyNfstAw/pub?gid=0&single=true&output=csv";
const webAppUrl = "https://script.google.com/macros/s/AKfycbwmXYL59ZvJOVPeKu9iCCpn5zjiWIR1GBrDSijNaaYdWPdk4bQ57t4F1svGBFI7nuwpmw/exec";
...
function atualizarSelectsDeItens() {
    ...
    selects.forEach(select => {
        ...
        itensOrdenados.forEach(obj => {
            ...
            option.dataset.categoria = obj.categoria || "";   // ← ADICIONAR ESTA LINHA
...
document.getElementById("formMovimentos").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        const colaboradorReceber = document.getElementById("colaboradorReceber").value.trim();  // ← ADICIONAR
...
if (!colaboradorReceber) {
    alert("Informe o colaborador a receber.");
    return;
}
        ...
        itensSecoes.forEach(secao => {
            ...
            id: itemID,
            item: itemNome,
            categoria: selectItem.options[selectItem.selectedIndex]?.dataset?.categoria || "",  // ← ADICIONAR
            tipo: secao.querySelector(".acao").value,
            quantidade: secao.querySelector(".quantidade").value,
            responsavel: responsavel,
            colaboradorReceber: colaboradorReceber,
            });
        });
...
function resetarFormulario() {
    ...
    document.getElementById("colaboradorReceber").value = "";
